...
- Confirm the user understands: if the machine is asleep/off at a scheduled time, that run is skipped, not caught up — the next scheduled run will simply run normally.
...
(or the `.ps1` equivalent on Windows). Report: config path, scheduler entry installed, and that `/pr-watch-disable` removes it later.
